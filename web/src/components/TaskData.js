...
import { Box, Typography, TextField, Button, Grid, 
	Collapse, Container } from '@material-ui/core';
import DateTimeField from './DateTimeField';
import ActivityButton from './ActivityButton';
import CounterButton from './CounterButton';
import TimerDataField from './TimerDataField';
import { GetTaskData, UpdateTaskData, ChangeTaskStatus, ChangeTaskActivity, 
	GetTaskTimerValues } from './Request';
...
export default class TaskData extends React.Component {
	...
	changeTaskProperties(propertyName, value) {
		...
		if (handler != null) {
			handler(data)
				.then((response) => { this.getTaskData(); });
		}
	}
	...
	updateTimerValues() {
		GetTaskTimerValues({"id": this.taskID})
			.then((response) => {
				this.setState({
		...
		return <Container mt={2}>
			<Box>
				<Box p={4} mt={2} display="flex">
					<Box mr={2}>
						<Typography variant="h6" color="textPrimary">{TitleTaskDuration}</Typography>
					</Box>
					<TimerDataField days={this.state.timer_Days} hours={this.state.timer_Hours} 
						minutes={this.state.timer_Minutes} seconds={this.state.timer_Seconds} 
						active={this.state.active} />
				</Box>
				<Box p={4} mt={2} display="flex">
					<TextField id="standart-basic" 
						label="Task title" value={this.state.title} 
				...
				<Grid p={4} mt={2} container justify="space-between">
					<Grid item>
						<Box mr={2}>
							<Typography variant="h6" color="textPrimary">{TitleTaskStartDate}</Typography>
						</Box>
						<DateTimeField date={this.state.startDate} time={this.state.startTime} 
							InputProps={{ readonly: true, }}
							colon=":" showSeconds />
					</Grid>
					<Grid item>
						<Box mr={2}>
							<Typography variant="h6" color="textPrimary">{TitleTaskEndDate}</Typography>
						</Box>
						<DateTimeField date={this.state.endDate} time={this.state.endTime} 
							InputProps={{ readonly: true, }}
							colon=":" showSeconds />
					</Grid>
				</Grid>
				...
				<Box p={4} mt={2} display="flex">
					<Box mr={2}>
						<Typography variant="h6" color="textPrimary">{TitleTaskPriority}</Typography>
					</Box>
					<Box>
						<CounterButton max={MaxProirityValue} min={MinPriorityValue} count={this.state.priority} 
							onClick={(value) => { this.setState({priority: value}); }} />
					</Box>
				</Box>
				<Box p={4} mt={2} display="flex" flexDirection="row">
					<Box mr={2}>
						<ActivityButton isActive={this.state.open} buttonNameOn={"open"} buttonNameOff={"close"}
							onChange={(value) => { this.changeTaskProperties("open", value); }} />
					</Box>
					<Box ml={2}>
						<ActivityButton isActive={this.state.active} buttonNameOn={"activate"} buttonNameOff={"deactivate"}
							onChange={(value) => { this.changeTaskProperties("active", value); }} />
					</Box>
					<Box ml={2}>
					...
					<Box ml={2}>
						<ActivityButton isActive={this.state.descrFieldCollaped} buttonNameOn={"show description"} 
							buttonNameOff={"hide description"} colorChange="no"
							onChange={(value) => {this.setState({descrFieldCollaped: value}); }} />
					</Box>
				</Box>
				<Box p={4} mt={2}>
					<Box p={4}>
						<Collapse timeout='auto' unmountOnExit in={this.state.descrFieldCollaped}> 
							<TextField id="outlined-multiline-static" rows={30} variant="outlined" 
